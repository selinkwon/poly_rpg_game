...
public class Guild{
	...
	public void setGuild() {
		Player temp = new Player("파장세린", 3, 1000, 100, 10, 0);
		this.guildList.add(temp);
		temp = new Player("오크보석", 1, 80, 7, 3, 0);
		this.guildList.add(temp);
		temp = new Player("깍두기예지", 1, 50, 3, 1, 0);
		this.guildList.add(temp);
		temp = new Player("딜러도하", 2, 70, 10, 2, 0);
		this.guildList.add(temp);
		temp = new Player("탱커지은", 1, 200, 4, 8, 0);
		this.guildList.add(temp);
		temp = new Player("힐러진경", -1, 1, 1, -1, 0);
		this.guildList.add(temp);
		for (int i = 0; i < 4; i++) {
	...
	private void buyUnit() {
		...
		name += n2[ran.nextInt(n1.length)];
		name += n3[ran.nextInt(n1.length)];
	...
	private void removeUnit() {
		...
		int sel = Integer.parseInt(Tool.input());
	...
	private void partyChange() {
		...
		int partyNum = Integer.parseInt(Tool.input());
		printAllUnitStaus();
		System.out.println("참가할 번호를 입력하세요 ");
		int guildNum = Integer.parseInt(Tool.input());
	...
	private void addPartyUnit() {
		while(true) {
			if (this.partyList.length < this.PARTY_SIZE) {
				...
				int guildNum = Integer.parseInt(Tool.input()) - 1;
	...
	private void deletePartyUnit() {
		while (true) {
			if(this.partyList.length>0) {
				printParty();
				System.out.println("삭제할 번호를 입력하세요 ");
				int sel = Integer.parseInt(Tool.input()) - 1;
